...
namespace Microsoft.Xaml.Interactions.Media
{
    ...
    public sealed class ControlStoryboardAction : DependencyObject, IAction
    {
        ...
        private static readonly System.Collections.Generic.Dictionary<Storyboard, bool> _storyboardIsPaused = new System.Collections.Generic.Dictionary<Storyboard, bool>();
                ...
                case ControlStoryboardOption.TogglePlayPause:
                    {
                        ...
                        {
                            _storyboardIsPaused[this.Storyboard] = false;
                            this.Storyboard.Begin();
                        }
                        else if (_storyboardIsPaused[this.Storyboard])
                        {
                            _storyboardIsPaused[this.Storyboard] = false;
                        ...
                        {
                            _storyboardIsPaused[this.Storyboard] = true;
        ...
        {
            Storyboard oldStoryboard = (Storyboard)args.OldValue;
            if (oldStoryboard != null)
            {
                _storyboardIsPaused.Remove(oldStoryboard);
            }

            Storyboard newStoryboard = (Storyboard)args.NewValue;
            if (newStoryboard != null && _storyboardIsPaused.ContainsKey(newStoryboard) == false)
            {
                _storyboardIsPaused.Add(newStoryboard, true);
            }
        }
    }
}